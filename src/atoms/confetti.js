...
export class ConfettiParticle {
    ...
    draw() {
        ...
        this.context.lineTo(
            this.x + this.tilt,
            this.y + this.tilt + this.radius / 5,
        );
        this.context.stroke();
    }
}
...
export class Confetti extends LitElement {
    ...
    renderToCanvas() {
        ...
        for (let i = this.particles.length - 1; i >= 0; i--) {
            ...
            particle.y +=
                (Math.cos(particle.d) + 3 + particle.radius / this.speedratio) /
                2;
            particle.tilt =
                Math.sin(
                    particle.tiltAngle - particle.tiltAngleDecremental / 3,
                ) * 15;

            // If a confetti has fluttered out of view we reset the particle and remove it if necessary.
            if (
                particle.x > this.canvasWidth + this.particleStartOffset ||
                particle.x < -this.particleStartOffset ||
