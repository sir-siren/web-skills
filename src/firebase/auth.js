...
export class Auth extends HTMLElement {
    ...
    setup() {
        ...
        this.completedSkills = (() => {
            try {
                return JSON.parse(
                    localStorage.getItem(StorageNames.completedSkills),
                );
            } catch (err) {
                return [];
            }
        ...
        this.user = (() => {
            try {
                return JSON.parse(
                    localStorage.getItem(StorageNames.sessionUser),
                );
            } catch (err) {
                return null;
            }
    ...
    setUser(user) {
        ...
        this.user = user;
        auth.dispatchEvent(
            new CustomEvent(AuthEvents.authStateChanged, { detail: user }),
        );
    }
    ...
    setCompletedSkills(skills) {
        skills = skills || [];
        localStorage.setItem(
            StorageNames.completedSkills,
            JSON.stringify(skills),
        );

        this.completedSkills = skills;
        auth.dispatchEvent(
            new CustomEvent(AuthEvents.completedSkillsChanged, {
                detail: skills,
            }),
        );
    }
    ...
    async addCompletedSkill(skillId) {
        if (!this.hasCompletedSkill(skillId)) {
            return await this.updateCompletedSkills([
                ...this.completedSkills,
                skillId,
            ]);
        }
    ...
    async removeCompletedSkill(skillId) {
        if (this.hasCompletedSkill(skillId)) {
            return await this.updateCompletedSkills(
                this.completedSkills.filter((id) => id !== skillId),
            );
        }
